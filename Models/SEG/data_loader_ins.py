...
class Dataset(data.Dataset):
        ...
        with open(self.filename, 'r', encoding='UTF-8') as f:
            ...
            for index, lin in enumerate(lines):
                if len(lin.split('\t')) == 3:
                    ...
                    assert len(e1_mask) == self.max_len
                    assert len(e2_mask) == self.max_len

                    return token_idx, att_mask, pos1s, pos2s, pos_mask, e1_mask, e2_mask, label
        ...
        with open(self.filename, 'r', encoding='UTF-8') as f:
            ...
            for index, lin in enumerate(lines):
                if len(lin.split('\t')) == 3:
                    ...
                    self.data.append((token_idx, att_mask, e1_mask, e2_mask, pos1s, pos2s, pos_mask, label))
    ...
    def __getitem__(self, index):
        ...
        token_idxes, att_masks, e1_masks, e2_masks, pos1es, pos2es, pos_masks, labels = \
            [torch.tensor(x, dtype=torch.long).unsqueeze(0) for x in _data]
        return token_idxes, att_masks, e1_masks, e2_masks, pos1es, pos2es, pos_masks, labels


def collate_fn(X):
    X = list(zip(*X))  # 解压
    token_idxes, att_masks, e1_masks, e2_masks, pos1es, pos2es, pos_masks, labels = X

    token_idxes = torch.cat(token_idxes, 0)
    att_masks = torch.cat(att_masks, 0)
    e1_masks = torch.cat(e1_masks, 0)
    e2_masks = torch.cat(e2_masks, 0)
    pos1es = torch.cat(pos1es, 0)
    pos2es = torch.cat(pos2es, 0)
    pos_masks = torch.cat(pos_masks, 0)
    labels = torch.stack(labels)

    return token_idxes, att_masks, e1_masks, e2_masks, pos1es, pos2es, pos_masks, labels.squeeze(1)
